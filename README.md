...
- 详细介绍可观看：https://www.bilibili.com/opus/1220065312502710326
- *漫威超级英雄*
    - 701.69. 新增关键字动作「治愈」。治愈某永久物某数量的伤害指移去永久物上标记的该数量的伤害。治愈某永久物指移去永久物上标记的所有伤害。（一般只有生物上会标记伤害。）
    - 702.193. 新增关键字异能超能，它为其后的起动式异能之效应增添以下规则：「如果此永久物是于本回合进场，则此异能的费用会减少等同于此永久物的法术力费用。只能起动一次。」
    - 702.194. 新增关键字异能联合。「联合N」意指，「你可以横置任意数量由你操控且力量总和等于或大于N的生物，以作为施放此咒语的额外费用。」
    - 601.5. 施放咒语时，如果能选择可选项A的前提条件是在施放流程的稍后时间才能选择的B，那可以视为任意合法的B来选择A。（复仇者先锋美国队长选择卸装哪个武具可能导致不同的分配目标数。）
    - 603.12a 这条涉及「支付任意次数费用时」触发之异能的规则，现在覆盖支付指定数量次数的情况了。（神射手鹰眼）
    - 700.16. 英杰生物是白和/或红且非反派的传奇生物。（雷神之锤妙尔尼尔）
    - 701.44c、701.50b、702.2e、702.15c、702.80b、702.90d 一些规则提及「如果物件已改变区域，则使用最后已知信息来决定谁在勘察/筹谋/它是否具有死触/系命/干枯/侵染。」现在它们改为了「如果物件已不在原先区域」，覆盖了跃离的情形。
    - 701.50e 刺探0则没有刺探事件发生。
    - 702.54b、702.57a、702.142a、702.177a、702.178a 删去了嗜血X、预报、炫威、竭绝、速度极限是某类异能的特殊形式的表述，嗜血X写为嗜血的变化形式。此为非功能性变更。
    - 702.122c 新增了载具「由某生物搭载」的定义。和你想的一样。（巴夫雷尔与法兰）
    - 707.12a 创造多个复制品且叙述某牌手「可以施放」这些物件的效应，允许该牌手为每个此类物件分别选择是否施放。（赫尔穆特·泽莫男爵）
    - 712.16. 融合永久物以及有双面组件的结聚永久物不能被翻为牌面朝下。如果有效应试图如此作，什么都不会发生，该效应也不会改变其特征或可复制特征值。
    - 800.4i 如果效应试图获取某已离开游戏牌手于其上回合执行的操作，则直到该牌手本该开始回合的时间，该效应可以获取这些操作。比如说，ABCD四位牌手按顺序进行回合，A在B的回合离开游戏，则直到D结束回合前，都可以获取A于其上回合执行的操作。（复仇行动）
    - 新预定义衍生物：振金衍生物（具有不灭和「{T}：加{C}。此法术力不能用于施放非神器咒语」的无色振金衍生神器）。
    - 新副类别：计划[Plan]、振金[Vibranium]、永恒族[Eternal]、伽马人[Gamma]、异人族[Inhuman]、克里人[Kree]、希阿[Shi’ar]、斯克鲁人[Skrull]、密探[Spy]。
    - 新词汇表条目：治愈、超能、联合、振金衍生物、英杰

### 20260426

- 详细介绍可观看：https://www.bilibili.com/opus/1194907524346675200
- *忍者神龟*
    - 111.10v：新的预定义衍生物：突变剂。
    - 207.2c：新异能提示：消失。
    - 506.3d, 508.4d, 702.49c：如果于战斗阶段中，宣告阻挡者后（包括宣告阻挡者步骤、战斗伤害步骤、战斗结束步骤），有生物进场且正进行攻击，那么它为未受阻挡状态。它直到移出战斗、效应使它成为受阻挡或战斗结束，都是未受阻挡的生物。
    - 508.4：进场且正进行攻击的生物直到移出战斗或战斗结束，都是进行攻击的生物。（一直以来都是这样，只是写出来了。）
    - 702.124：新的拍档叙述：角色选择。
    - 新的副类别：突变剂、乌创。
    - 词汇表更新：奇袭。
- *斯翠海文的秘密*
    - 107.3e：如果某物检查堆叠上咒语之X的数值，且有另一物修改了X，那么前者会检查到X的当前值。（黄铜无限镜、无拘滋养）
    - 118.7e：明确如果在减少费用时，减少了类似{C/G}的费用，那么左侧减少{C}而不是减少{1}。
    - 207.2c：新异能提示：连辞、炫示、输注。
    - 508.4a：把某生物放进战场且正在攻击某位牌手或某个永久物，而后者不能被合法攻击时，生物将进战场但不是攻击生物。以前这条规则仅说了效应把生物放进战场的情况，现在删除了效应的表述，从而包含了奇袭这种因咒语结算而进场的情况。
    - 603.10a：有一些异能是否触发，需要根据事件发生前的情况，「回溯」确定是否触发。其中包括「离开战场异能」的情况。原先，「每当牺牲时」触发的异能非显式地包含在「离开战场异能」内，但现在将这种情况单独摘出了。（刻洛死亡僧侣马兹瑞）
    - 701.17d：规范描述。如果磨一张牌并给予牌手使用或施放该牌的许可，但因替代性效应而磨了多张牌，则此许可对每一张磨掉的牌都生效。（浮夸的布鲁瓦）
    - 702.155b, 714.3a, 714.3b：「传纪进场时上面有一个学问指示物」在传纪刚推出时是规则，现在更新为了传纪的固有静止式异能（与鹏洛客和战役进行了统一；感谢鼠鼠就此事询问Matt Tabak推动规则简明化）。跳读传纪带有指示物也更新为了固有静止式异能。这项改动产生的影响包括：
        - 如果倍产旺季在场上，普通传纪进场时上面会有两个学问指示物了。前两个章节异能同时触发，传纪的操控者选择这两个异能结算的顺序；
        - 如果倍产旺季在场上，跳读传纪进场时上面会有两倍于所选数量的学问指示物。只有章节编号等同于其上学问指示物数量的那一个章节异能会触发；
        - 如果腥红之月在场上，克撒传进场时上面不会有学问指示物。在腥红之月离开战场后，克撒传操控者的下一个第一个行动阶段开始时，会在克撒传上放上第一个学问指示物，克撒传会获得「{T}：加{C}。」
    - 702.190 修补奇袭异能的语病。
    - 702.191, 702.192：新关键字异能：递增、定式。
    - 722：新机制：备法牌。
    - 新的副类别：书籍、德连、长颈鹿
    - 词汇表更新：递增、定式、备法牌、备法咒语、已备法、已撤法。删除词汇表条目：巴黎再调度、温哥华再调度。
